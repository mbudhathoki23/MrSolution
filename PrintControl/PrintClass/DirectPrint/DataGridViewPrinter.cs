...
{
    private static int
        _currentRow; // A static parameter that keep track on which Row (in the DataGridView control) that should be printed
    ...
    private float
        _currentY; // A parameter that keep track on the y coordinate of the page, so the next object to be printed will start from this y coordinate
    ...
    public DataGridViewPrinter(DataGridView aDataGridView, System.Drawing.Printing.PrintDocument aPrintDocument, bool centerOnPage,
        bool withTitle, string aTitleText, Font aTitleFont, Color aTitleColor, bool withPaging)
    {
    ...
    private void Calculate(Graphics g)
    {
        if (_pageNumber == 0) // Just calculate once
        {
            ...
            for (var i = 0; i < _theDataGridView.Columns.Count; i++)
            {
                var tmpFont = _theDataGridView.ColumnHeadersDefaultCellStyle.Font ??
                ...
                for (var j = 0; j < _theDataGridView.Rows.Count; j++)
                {
                    ...
                    tmpSize = g.MeasureString(_theDataGridView.Rows[j].Cells[i].EditedFormattedValue.ToString(),
                        tmpFont);
                    if (tmpSize.Width > tmpWidth) tmpWidth = tmpSize.Width;
                }

                if (_theDataGridView.Columns[i].Visible) _theDataGridViewWidth += tmpWidth;
            ...
            var mEndPoint = _theDataGridView.Columns.Count;
            ...
            var mTempWidth = _theDataGridViewWidth;
            var mTempPrintArea = _pageWidth - (float)_leftMargin - _rightMargin;
    ...
    private void DrawHeader(Graphics g)
    {
        ...
        {
            _pageNumber++;
            var pageString = "Page " + _pageNumber;
            ...
            var pageStringRectangle = new RectangleF(_leftMargin, _currentY,
                _pageWidth - (float)_rightMargin - _leftMargin, g.MeasureString(pageString, pageStringFont).Height);
        ...
        if (_isWithTitle)
        {
            ...
            titleFormat.Alignment = _isCenterOnPage ? StringAlignment.Center : StringAlignment.Near;
            var titleRectangle = new RectangleF(_leftMargin, _currentY, _pageWidth - (float)_rightMargin - _leftMargin,
        ...
        var headerForeColor = _theDataGridView.ColumnHeadersDefaultCellStyle.ForeColor;
        headerForeColor = headerForeColor.IsEmpty switch
        {
            // If there is no special HeaderFore style, then use the default DataGridView style
            true => _theDataGridView.DefaultCellStyle.ForeColor,
            _ => headerForeColor
        };
        var headerForeBrush = new SolidBrush(headerForeColor);

        // Setting the HeaderBack style
        var headerBackColor = _theDataGridView.ColumnHeadersDefaultCellStyle.BackColor;
        headerBackColor = headerBackColor.IsEmpty switch
        {
            // If there is no special HeaderBack style, then use the default DataGridView style
            true => _theDataGridView.DefaultCellStyle.BackColor,
            _ => headerBackColor
        };
        ...
        var headerFont = _theDataGridView.ColumnHeadersDefaultCellStyle.Font ?? _theDataGridView.DefaultCellStyle.Font;
        ...
        for (var i = (int)_mColumnPoints[_mColumnPoint].GetValue(0);
             i < (int)_mColumnPoints[_mColumnPoint].GetValue(1);
             i++)
        {
            if (!_theDataGridView.Columns[i]
                    .Visible) continue; // If the column is not visible then ignore this iteration
            ...
            if (_theDataGridView.RowHeadersBorderStyle !=
                DataGridViewHeaderBorderStyle.None) // Draw the cell border only if the HeaderBorderStyle is not None
                ...
                .Visible) // Print the cells of the CurrentRow only if that row is visible
            {
                    ...
                    .IsEmpty) // If the there is no special RowBack style of the CurrentRow, then use the default one associated with the DataGridView control
                {
                    ...
                    rowAlternatingBackBrush =
                        new SolidBrush(_theDataGridView.AlternatingRowsDefaultCellStyle.BackColor);
                }
                ...
                var currentX = (float)_leftMargin;
                if (_isCenterOnPage)
                    currentX += (_pageWidth - (float)_rightMargin - _leftMargin - _mColumnPointsWidth[_mColumnPoint]) /
                                2.0F;

                // Calculating the entire CurrentRow bounds
                var rowBounds = new RectangleF(currentX, _currentY, _mColumnPointsWidth[_mColumnPoint],
                    _rowsHeight[_currentRow]);
                ...
                for (var currentCell = (int)_mColumnPoints[_mColumnPoint].GetValue(0);
                     currentCell < (int)_mColumnPoints[_mColumnPoint].GetValue(1);
                     currentCell++)
                {
                    if (!_theDataGridView.Columns[currentCell]
                            .Visible) continue; // If the cell is belong to invisible column, then ignore this iteration

                    // Check the CurrentCell alignment and apply it to the CellFormat
                    if (_theDataGridView.Columns[currentCell].DefaultCellStyle.Alignment.ToString().Contains("Right"))
                        cellFormat.Alignment = StringAlignment.Far;
                    else if (_theDataGridView.Columns[currentCell].DefaultCellStyle.Alignment.ToString()
                             .Contains("Center"))
                        cellFormat.Alignment = StringAlignment.Center;
                    else
                        cellFormat.Alignment = StringAlignment.Near;
                    ...
                    // Printing the cell text
                    g.DrawString(_theDataGridView.Rows[_currentRow].Cells[currentCell].EditedFormattedValue.ToString(),
                        rowFont, rowForeBrush, cellBounds, cellFormat);

                    // Drawing the cell bounds
                    if (_theDataGridView.CellBorderStyle !=
