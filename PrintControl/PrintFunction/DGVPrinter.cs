...
namespace PrintControl.PrintFunction; //AllocationRequest
...
public class DGVPrinter
{
    ...
    private void SetupPrint()
    {
        if (EnableLogging)
        {
            ...
            Logger.LogInfoMsg(string.Format("Initial Printer Margins are {0}, {1}, {2}, {3}", m.Left, m.Right,
                m.Top, m.Bottom));
        }
        ...
        */
        ...
        if (!EmbeddedPrinting)
        {
            ...
            if (EnableLogging)
            {
                Logger.LogInfoMsg(string.Format("Printer 'Hard' X limit is {0} and 'Hard' Y limit is {1}", hardx,
                    hardy));
                Logger.LogInfoMsg(string.Format(
    ...
    private void SetupPrintRange()
    {
        ...
        if (EnableLogging)
            Logger.LogInfoMsg(string.Format("Grid Printout Range is {0} columns", colstoprint.Count));
    ...
    private void RecalcRowHeights(Graphics g, int colindex, float newcolwidth)
    {
        ...
        for (var i = 0; i < rowstoprint.Count; i++)
        {
            cell = rowstoprint[i].row.Cells[((DataGridViewColumn)colstoprint[colindex]).Index];
            ...
            if (RowHeightSetting.DataHeight == RowHeight)
            {
                ...
                // get column style
                var colstyle = GetStyle(rowstoprint[i].row, (DataGridViewColumn)colstoprint[colindex]);
                ...
                var size = calccellsize(g, cell, colstyle, newcolwidth, colwidthsoverride[colindex], currentformat);
    ...
    private void measureprintarea(Graphics g)
    {
        ...
        for (i = 0; i < colstoprint.Count; i++)
        {
            ...
            else if (ColumnWidthSetting.CellWidth == ColumnWidth || ColumnWidthSetting.Porportional == ColumnWidth)
            {
                ...
                size = calccellsize(g, col.HeaderCell, headercolstyle, usewidth, colwidthsoverride[i],
    ...
    private bool PrintPage(Graphics g)
    {
        ...
        CurrentPage++;
        if (EnableLogging)
            Logger.LogInfoMsg(string.Format("Print Page processing page {0} -----------------------", CurrentPage));
        ...
        {
            if (EnableLogging)
                Logger.LogInfoMsg(string.Format("Print Page skipping page {0} part {1}", CurrentPage,
                    currentpageset + 1));
            ...
            // log rows skipped
            if (EnableLogging)
                Logger.LogInfoMsg(string.Format("Print Page skipped rows {0} to {1}", firstrow, currentrow));
        ...
        if (EnableLogging)
        {
            Logger.LogInfoMsg(string.Format("Print Page printing page {0} part {1}", CurrentPage,
                currentpageset + 1));
            var m = pagesets[currentpageset].margins;
            Logger.LogInfoMsg(string.Format("Current Margins are {0}, {1}, {2}, {3}", m.Left, m.Right, m.Top,
                m.Bottom));
        }
        ...
        ImbeddedImageList.Where(p => p.ImageLocation == Location.Absolute).DrawImbeddedImage(g,
            pagesets[currentpageset].printWidth, pageHeight, pagesets[currentpageset].margins);
        ...
        if (PrintHeader)
        {
            ...
            ImbeddedImageList
                .Where(p => p.ImageLocation == Location.Header).DrawImbeddedImage(g,
                    pagesets[currentpageset].printWidth, pageHeight,
        ...
        if (PrintFooter)
        {
            ...
            ImbeddedImageList
                .Where(p => p.ImageLocation == Location.Footer).DrawImbeddedImage(g,
                    pagesets[currentpageset].printWidth, pageHeight,
                    pagesets[currentpageset].margins);
    ...
    ///     are: MrFormatFlags - NoWrap, LineLimit, NoClip
    /// </summary>
