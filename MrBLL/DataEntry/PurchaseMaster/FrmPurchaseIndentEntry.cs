...
public partial class FrmPurchaseIndentEntry : MrForm
{
    #region --------------- PURCHASE INVOICE ENTRY ---------------
    public FrmPurchaseIndentEntry(bool zoom, string txtZoomVno, bool provisionVoucher = false, string invoiceType = "NORMAL")
    {
        InitializeComponent();
        _indentRepository = new PurchaseIndentRepository();
    ...
    private void FrmPurchaseIndentEntry_KeyPress(object sender, KeyPressEventArgs e)
    {
        _getKeys = e;
    ...
    private void FrmPurchaseIndentEntry_Shown(object sender, EventArgs e)
    {
        if (BtnNew.Enabled)
        {
            BtnNew.Focus();
    ...
    private void ClearControl()
    {
        ...
        if (!_isZoom)
        {
            if (_actionTag.Equals("SAVE")) TxtVno.GetCurrentVoucherNo("PB", _numberScheme);
            if (BtnNew.Enabled)
            {
    ...
    private int SavePurchaseInvoice()
    {
        try
        {
            if (_actionTag is "SAVE") TxtVno.GetCurrentVoucherNo("PB", _numberScheme);
            _indentRepository.PinMaster.PIN_Invoice = TxtVno.Text;
            _indentRepository.PinMaster.PIN_Date = DateTime.Parse(MskDate.Text);
            _indentRepository.PinMaster.PIN_Miti = MskMiti.Text;
            _indentRepository.PinMaster.Person = TxtVendor.Text;
            _indentRepository.PinMaster.Sub_Ledger = _subLedgerId;
            _indentRepository.PinMaster.AgentId = _agentId;
            _indentRepository.PinMaster.Cls1 = _departmentId;
            _indentRepository.PinMaster.Cls2 = 0;
            _indentRepository.PinMaster.Cls3 = 0;
            _indentRepository.PinMaster.Cls4 = 0;
            _indentRepository.PinMaster.EnterBY = ObjGlobal.LogInUser;
            _indentRepository.PinMaster.EnterDate = DateTime.Now;
            _indentRepository.PinMaster.ActionType = _actionTag;
            _indentRepository.PinMaster.R_Invoice = false;
            _indentRepository.PinMaster.Remarks = TxtRemarks.Text;

            _indentRepository.PinMaster.SyncBaseId = ObjGlobal.IsOnlineSync ? Guid.NewGuid() : Guid.Empty;
            _indentRepository.PinMaster.SyncGlobalId = ObjGlobal.IsOnlineSync ? Guid.NewGuid() : Guid.Empty;
            if (ObjGlobal.LocalOriginId != null)
            {
                _indentRepository.PinMaster.SyncOriginId = ObjGlobal.IsOnlineSync && ObjGlobal.LocalOriginId.IsValueExits()
                    ? ObjGlobal.LocalOriginId.Value
                    : Guid.Empty;
            }

            _indentRepository.PinMaster.SyncCreatedOn = DateTime.Now;
            _indentRepository.PinMaster.SyncLastPatchedOn = DateTime.Now;
            _indentRepository.PinMaster.GetView = DGrid;

            return _indentRepository.SavePurchaseInvoice(_actionTag);
        }
        catch (Exception ex)
        {
    ...
    {
        if (_setup.GetPrintVoucherList("PIN").Rows.Count > 0)
            new FrmDocumentPrint("Crystal", "PIN", TxtVno.Text, TxtVno.Text).ShowDialog();
    }
    ...
    private void FillInvoiceData(string voucherNo)
    {
        try
        {
            var dsPurchase = _indentRepository.ReturnPurchaseInvoiceDetailsInDataSet(voucherNo);
            if (dsPurchase.Tables.Count > 0)
                if (dsPurchase.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (dsPurchase.Tables[2].Rows.Count > 0) _dtProductTerm = dsPurchase.Tables[2];
                    if (dsPurchase.Tables[3].Rows.Count > 0) _dtBillTerm = dsPurchase.Tables[3];
                }
    ...
    {
        var frmDp = new FrmDocumentPrint("Crystal", "PB", TxtVno.Text, TxtVno.Text, TxtVno.Text)
        {
            Owner = ActiveForm
        };
        frmDp.ShowDialog();
    ...
    {
        TxtVno.Text = GetTransactionList.GetTransactionVoucherNo(_actionTag, MskDate.Text, "MED", "PB");
        if (_actionTag != "SAVE") FillInvoiceData(TxtVno.Text);
    }
    ...
    private void BtnSave_Click(object sender, EventArgs e)
    {
        ...
        if (IsValidInformation())
        {
            if (SavePurchaseInvoice() != 0)
            {
                ...
                CustomMessageBox.ActionSuccess(TxtVno.Text, "PURCHASE INDENT", _actionTag);
                ClearControl();
                if (_actionTag != "SAVE")
                {
                    TxtVno.Enabled = true;
            ...
            {
                CustomMessageBox.ErrorMessage($"ERROR OCCURS WHILE PURCHASE INDENT [{_actionTag}]");
            }
        }
        else
        {
    ...
    {
        var dtVoucher = _indentRepository.CheckVoucherExitsOrNot("PB_Master", "PB_Invoice", TxtVno.Text);
    ...
    private void CmbInvType_KeyPress(object sender, KeyPressEventArgs e)
    {
        if (e.KeyChar is (char)Keys.Space)
            SendKeys.Send("{F4}");
        else if (e.KeyChar is (char)Keys.Enter) SendKeys.Send("{TAB}");
    }
    ...
    {
        if (e.KeyCode is not Keys.Enter || TxtProduct.Enabled) return;
    ...
    private void TxtBillTermAmount_TextChanged(object sender, EventArgs e)
    {
        ...
        LblTotalLocalNetAmount.Text = LblTotalNetAmount.Text;
        LblNumberInWords.Text = ClsMoneyConversion.MoneyConversion(LblTotalLocalNetAmount.Text);
    }
    ...
    private readonly IPurchaseIndentRepository _indentRepository;
