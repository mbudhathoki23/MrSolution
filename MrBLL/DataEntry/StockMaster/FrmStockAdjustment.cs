...
public partial class FrmStockAdjustment : MrForm
{
    // STOCK ADJUSTMENT

    #region --------------- STOCK ADJUSTMENT ---------------

    public FrmStockAdjustment(bool zoomFrm, string txtZoomVno)
    {
        InitializeComponent();
    ...
    {
        _getDesign.GetStockAdjustmentEntryDesign(DGrid);
        ...
        CmbType = new MrGridComboBox(DGrid);
        CmbType.KeyDown += (_, e) =>
        {
    ...
    private void AdjustControlsInDataGrid()
    {
        ...
        columnIndex = DGrid.Columns["GTxtType"].Index;
        CmbType.Size = DGrid.GetCellDisplayRectangle(columnIndex, currentRow, true).Size;
        CmbType.Location = DGrid.GetCellDisplayRectangle(columnIndex, currentRow, true).Location;
        CmbType.TabIndex = columnIndex;
    ...
    private void EnableGridControl(bool isEnable = false)
    {
        ...
        CmbType.Enabled = isEnable;
        CmbType.Visible = isEnable;
    ...
    private void ControlClear()
    {
        Text = _actionTag.IsValueExits()
            ? $@"STOCK ADJUSTMENT VOUCHER ENTRY [{_actionTag}]"
            : @"STOCK ADJUSTMENT VOUCHER ENTRY";
    ...
    private void SetProductInfo(long getProductId)
    {
        ...
        TxtAltQty.Text = _conAltQty.ToString(ObjGlobal.SysAmountFormat);
    ...
    private void AddDataToGridDetails(bool isGridUpdate)
    {
        ...
        DGrid.Rows[iRows].Cells["GTxtType"].Value = CmbType.Text;
