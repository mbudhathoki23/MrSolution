...
public partial class FrmSalesInvoiceEntry : MrForm
{
    //SALES INVOICE ENTRY

    #region --------------- SALES INVOICE ENTRY ---------------

    public FrmSalesInvoiceEntry(bool zoom, string txtZoomVno, bool provisionVoucher = false, string invoiceType = "NORMAL")
    {
        InitializeComponent();
    ...
    private void FrmSalesInvoiceEntry_KeyPress(object sender, KeyPressEventArgs e)
    {
        _getKeys = e;
    ...
    private void FrmSalesInvoiceEntry_Shown(object sender, EventArgs e)
    {
        if (BtnNew.Enabled)
        {
            BtnNew.Focus();
    ...
    {
        if (TxtQty.GetDecimal() is not 0 || !TxtQty.Enabled || !TxtProduct.IsValueExits())
        {
            return;
        }
    ...
    {
        switch (DGrid.RowCount)
        {
            case 0:
                this.NotifyValidationError(TxtProduct, "PLEASE SELECT PRODUCT LEDGER FOR OPENING");
                break;
            case 1 when TxtProduct.Enabled && TxtProduct.IsBlankOrEmpty() && DGrid.Rows[0].Cells["GTxtProductId"].Value.GetLong() is 0 && DGrid.Rows[0].Cells["GTxtProductId"].Value.GetLong() is 0:
            {
    ...
    private void SaveCashAndBankDetails()
    {
        try
        {
            if (_ledgerType.Contains(ledgerType))
            ...
            _financeEntry.SaveCashBankVoucher(_actionTag);
        }
        catch (Exception ex)
        {
    ...
    private int SaveSalesInvoice()
    {
        try
        {
            ...
            _salesInvoice.SbMaster.Invoice_Date = DateTime.Parse(MskDate.Text);
            _salesInvoice.SbMaster.Invoice_Miti = MskMiti.Text.Contains("/") ? MskMiti.Text : MskMiti.Text.Replace("-", "/");
            _salesInvoice.SbMaster.PB_Vno = TxtRefVno.Text;
            _salesInvoice.SbMaster.Vno_Miti = MskRefDate.Text.Contains("/") ? MskRefDate.Text : MskRefDate.Text.Replace("-", "/");
            _salesInvoice.SbMaster.Vno_Date = MskRefDate.GetEnglishDate(MskRefDate.Text).GetDateTime();
            _salesInvoice.SbMaster.Invoice_Time = DateTime.Now;
            _salesInvoice.SbMaster.Customer_Id = _ledgerId;
            ...
            _salesInvoice.SbMaster.ChqDate = _dtPartyInfo.Rows.Count > 0 && _dtPartyInfo.Rows[0]["ChequeNo"].IsValueExits() ? _dtPartyInfo.Rows[0]["ChequeDate"].GetDateTime() : DateTime.Now;

            _salesInvoice.SbMaster.Invoice_Type = CmbInvoiceType.Text;
            _salesInvoice.SbMaster.Invoice_Mode = "AIMS";
            _salesInvoice.SbMaster.Payment_Mode = CmbPaymentMode.Text;
            _salesInvoice.SbMaster.DueDays = TxtDueDays.Text.GetInt();
            _salesInvoice.SbMaster.DueDate = MskDueDays.MaskCompleted ? MskDueDays.GetEnglishDate(MskDueDays.Text).GetDateTime() : DateTime.Now;
            _salesInvoice.SbMaster.Agent_Id = _agentId > 0 ? _agentId : null;
            _salesInvoice.SbMaster.Subledger_Id = _subLedgerId > 0 ? _subLedgerId : null;
            _salesInvoice.SbMaster.SO_Invoice = TxtOrder.Text;
            _salesInvoice.SbMaster.SO_Date = _mskOrderDate.GetDateTime();
            _salesInvoice.SbMaster.SC_Invoice = TxtChallan.Text;
            _salesInvoice.SbMaster.SC_Date = _mskChallanDate.GetDateTime();
            _salesInvoice.SbMaster.Cls1 = _departmentId > 0 ? _departmentId : null;
            ...
            _salesInvoice.SbMaster.Cur_Rate = TxtCurrencyRate.Text.GetDecimal() > 0 ? TxtCurrencyRate.Text.GetDecimal() : 1;
            _salesInvoice.SbMaster.CounterId = 0;
            _salesInvoice.SbMaster.B_Amount = LblTotalBasicAmount.Text.GetDecimal();
            _salesInvoice.SbMaster.T_Amount = TxtBillTermAmount.Text.GetDecimal();
            _salesInvoice.SbMaster.N_Amount = LblTotalNetAmount.Text.GetDecimal();
            _salesInvoice.SbMaster.LN_Amount = LblTotalLocalNetAmount.Text.GetDecimal();
            _salesInvoice.SbMaster.Tender_Amount = 0;
            _salesInvoice.SbMaster.Return_Amount = 0;
            _salesInvoice.SbMaster.V_Amount = 0;
            _salesInvoice.SbMaster.Tbl_Amount = 0;
            ...
            if (DGrid.Rows.Count > 0)
            {
                foreach (DataGridViewRow row in DGrid.Rows)
                {
                    ...
                    list.SC_Invoice = row.Cells["GTxtChallanNo"].Value.GetString();
                    list.SC_SNo = row.Cells["GTxtChallanSno"].Value.GetInt();

                    list.Tax_Amount = row.Cells["GTxtTaxableAmount"].Value.GetDecimal();
                    list.V_Amount = row.Cells["GTxtVatAmount"].Value.GetDecimal();
            ...
            foreach (DataRow row in _dtBatchInfo.Rows)
            {
                ...
                _salesInvoice.AddInfos.Add(list);
                index++;
            }

            // IMAGE
            _salesInvoice.SbMaster.PAttachment1 = PAttachment1.Image.ConvertImage();
            _salesInvoice.SbMaster.PAttachment2 = PAttachment2.Image.ConvertImage();
            _salesInvoice.SbMaster.PAttachment3 = PAttachment3.Image.ConvertImage();
            _salesInvoice.SbMaster.PAttachment4 = PAttachment4.Image.ConvertImage();
            _salesInvoice.SbMaster.PAttachment5 = PAttachment5.Image.ConvertImage();

            // OTHER DETAILS
            _salesInvoice.SbOther.SB_Invoice = TxtVno.Text;
            _salesInvoice.SbOther.Transport = TxtTransport.Text;
            _salesInvoice.SbOther.VechileNo = TxtVechileNo.Text;
            _salesInvoice.SbOther.BiltyNo = TxtBiltyNo.Text;
            _salesInvoice.SbOther.Package = TxtPackage.Text;
            _salesInvoice.SbOther.BiltyDate = MskBiltyDate.GetDateTime();
            _salesInvoice.SbOther.BiltyType = CmbBiltyType.Text;
            _salesInvoice.SbOther.Driver = TxtDriver.Text;
            _salesInvoice.SbOther.PhoneNo = TxtPhoneNo.Text;
            _salesInvoice.SbOther.LicenseNo = TxtLicenseNo.Text;
            _salesInvoice.SbOther.MailingAddress = TxtMailingAddress.Text;
            _salesInvoice.SbOther.MCity = TxtMCity.Text;
            _salesInvoice.SbOther.MState = TxtMState.Text;
            _salesInvoice.SbOther.MCountry = TxtMCountry.Text;
            _salesInvoice.SbOther.MEmail = TxtMEmail.Text;
            _salesInvoice.SbOther.ShippingAddress = TxtShippingAddress.Text;
            _salesInvoice.SbOther.SCity = TxtSCity.Text;
            _salesInvoice.SbOther.SState = TxtSState.Text;
            _salesInvoice.SbOther.SCountry = TxtSCountry.Text;
            _salesInvoice.SbOther.SEmail = TxtSEmail.Text;
            _salesInvoice.SbOther.ContractNo = TxtContractNo.Text;
            _salesInvoice.SbOther.ContractDate = MskContractNoDate.GetDateTime();
            _salesInvoice.SbOther.ExportInvoice = TxtExportInvoiceNo.Text;
            _salesInvoice.SbOther.ExportInvoiceDate = MskExportInvoiceDate.GetDateTime();
            _salesInvoice.SbOther.ExportInvoice = TxtVendorOrderNo.Text;
            _salesInvoice.SbOther.BankDetails = TxtBankDetails.Text;
            _salesInvoice.SbOther.LcNumber = TxtLCNumber.Text;
            _salesInvoice.SbOther.CustomDetails = TxtCustomName.Text;
    ...
    private void EnableControl(bool isEnable = false)
    {
        ...
        {
            BtnEdit.Enabled = BtnDelete.Enabled = BtnReverse.Enabled = false;
            BtnEdit.Visible = BtnDelete.Visible = BtnReverse.Visible = false;
        }

        TxtVno.Enabled = BtnVno.Enabled = isEnable || _tagStrings.Contains(_actionTag);
        if (TxtVno.Enabled)
        {
            if (_actionTag is "REVERSE" || ObjGlobal.IsIrdRegister)
            {
        ...
        // For Ird Disable
        if (MskDate.Enabled && ObjGlobal.IsIrdRegister)
        {
        ...
        // Sales Challan Enable
        TxtChallan.Enabled = ObjGlobal.SalesChallanEnable && isEnable;
        BtnChallanNo.Enabled = TxtChallan.Enabled;
        MskChallanDate.Enabled = MskChallanMiti.Enabled = false;

        // Sales Order Enable
        TxtOrder.Enabled = isEnable && ObjGlobal.SalesOrderEnable;
        BtnOrderNo.Enabled = TxtOrder.Enabled;
        ...
        // Currency Enable
        TxtCurrency.Enabled = isEnable && ObjGlobal.SalesCurrencyEnable;
        BtnCurrency.Enabled = TxtCurrencyRate.Enabled = TxtCurrency.Enabled;

        // SubLeder Enable
        TxtSubledger.Enabled = isEnable && ObjGlobal.SalesSubLedgerEnable;
        BtnSubledger.Enabled = TxtSubledger.Enabled;

        // Department Enable
        TxtDepartment.Enabled = BtnDepartment.Enabled = isEnable && ObjGlobal.SalesDepartmentEnable;
        BtnDepartment.Enabled = TxtDepartment.Enabled;

        // Agent Enable
        TxtAgent.Enabled = BtnAgent.Enabled = isEnable && ObjGlobal.SalesAgentEnable;
        BtnAgent.Enabled = TxtAgent.Enabled;
        ...
        // Remarks Enable
        TxtRemarks.Enabled = (ObjGlobal.SalesRemarksEnable && isEnable) || _tagStrings.Contains(_actionTag);
        BtnSave.Enabled = BtnCancel.Enabled = isEnable;

        if (!BtnSave.Enabled && _tagStrings.Contains(_actionTag))
        {
    ...
    private void SetProductInfo(bool isUpdate = false)
    {
        ...
        foreach (DataRow row in _dtProduct.Rows)
        {
            ...
            if (!_isRowUpdate)
            {
                ...
                _conAltQty = row["PAltConv"].GetDecimal();
    ...
    private bool IsValidInformation()
    {
        ...
        if (MskDate.MaskCompleted)
        {
            if (MskDate.Text.IsDateExits("D"))
            {
                ...
                {
                    this.NotifyValidationError(MskDate, $"ENTER DATE MUST BE BETWEEN {ObjGlobal.CfStartAdDate} AND {ObjGlobal.CfEndAdDate}");
                    return false;
                }
            }
            else
        ...
        if (MskMiti.MaskCompleted)
        {
            if (MskMiti.Text.IsDateExits("M"))
            {
                ...
                {
                    this.NotifyValidationError(MskMiti, $"ENTER MITI MUST BE BETWEEN {ObjGlobal.CfStartBsDate} AND {ObjGlobal.CfEndBsDate}");
                    return false;
                }
            }
            else
    ...
    private IFinanceEntry _financeEntry;
    private IMasterSetup _master;
    private ISalesDesign _design;
