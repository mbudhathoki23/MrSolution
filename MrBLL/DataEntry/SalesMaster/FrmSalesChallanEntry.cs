...
public partial class FrmSalesChallanEntry : MrForm
{
    //SALES CHALLAN ENTRY

    #region --------------- SALES CHALLAN ENTRY ---------------

    public FrmSalesChallanEntry(bool zoom, string txtZoomVno, bool provisionVoucher = false, string invoiceType = "NORMAL")
    {
        InitializeComponent();
    ...
    private void FrmSalesChallanEntry_Load(object sender, EventArgs e)
    {
        if (_isZoom && _txtZoomVno.IsValueExits())
        {
            FillChallanInvoiceData(_txtZoomVno);
        }
        ObjGlobal.GetFormAccessControl([BtnNew, BtnEdit, BtnDelete], this.Tag);
    }

    private void FrmSalesChallanEntry_KeyPress(object sender, KeyPressEventArgs e)
    {
        _getKeys = e;
    ...
    private void FrmSalesChallanEntry_Shown(object sender, EventArgs e)
    {
        if (BtnNew.Enabled)
        {
            BtnNew.Focus();
    ...
    {
        BtnNew.PerformClick();
        TxtVno.Text = GetTransactionList.GetTransactionVoucherNo(_actionTag, MskDate.Text, "MED", "SC");
        FillChallanInvoiceData(TxtVno.Text);
        MskMiti.Focus();
    }
    ...
    {
        var voucherNo = GetTransactionList.GetTransactionVoucherNo(_actionTag, MskDate.Text, "MED", "SC");
    ...
    {
        TxtQuotationNo.Text = GetTransactionList.GetTransactionVoucherNo(_actionTag, MskDate.Text, "MED", "SC", "OSC");
        FillQuotationData(TxtQuotationNo.Text);
    ...
    private void TxtRemarks_KeyDown(object sender, KeyEventArgs e)
    {
        if (e.KeyCode is Keys.Enter)
        {
            ...
            {
                TxtRemarks.WarningMessage("REMARKS IS REQUIRED FOR CHALLAN..!!");
            }
            else
            {
                BtnSave.Focus();
    ...
    private void BtnSave_Click(object sender, EventArgs e)
    {
        ...
        if (IsValidInformation())
        {
            ...
            if (SaveSalesChallan() != 0)
            {
                ...
                this.NotifySuccess($@"{TxtVno.Text} SALES CHALLAN NUMBER {_actionTag} SUCCESSFULLY..!!");
                ClearControl();
            ...
            {
                this.NotifyError($@"{TxtVno.Text} ERROR OCCURS WHILE SALES CHALLAN {_actionTag}..!!");
            }
        }
    }
    ...
    private void TxtVno_Validating(object sender, CancelEventArgs e)
    {
        if (TxtVno.IsValueExits())
        {
            var dtVoucher = _entry.CheckVoucherNoExitsOrNot("AMS.SC_Master", "SC_Invoice", TxtVno.Text);
            if (dtVoucher.RowsCount() > 0 && _actionTag.Equals("SAVE"))
            {
                this.NotifyValidationError(TxtVno, "CHALLAN NUMBER ALREADY EXITS..!!");
            }
            else if (dtVoucher.Rows.Count <= 0 && !_actionTag.Equals("SAVE"))
            {
                this.NotifyValidationError(TxtVno, "CHALLAN NUMBER NOT EXITS..!!");
            }
        }
        ...
        if (_actionTag.IsValueExits() && TxtVno.IsBlankOrEmpty())
        {
            ...
            {
                TxtVno.WarningMessage("CHALLAN VOUCHER NUMBER IS REQUIRED");
                return;
            }
        }
    }
    ...
    private void TxtQuotation_KeyDown(object sender, KeyEventArgs e)
    {
        ...
        {
            if (ObjGlobal.SalesQuotationMandatory && TxtQuotationNo.IsBlankOrEmpty())
            {
                TxtQuotationNo.WarningMessage("SALES GRN NUMBER IS REQUIRED FOR CHALLAN");
                return;
            }
            TxtCustomer.Focus();
        }
    ...
    private void TxtQuotation_Validating(object sender, CancelEventArgs e)
    {
        if (TxtQuotationNo.IsBlankOrEmpty() && TxtQuotationNo.Enabled && ObjGlobal.SalesQuotationMandatory)
        {
            if (_actionTag.IsValueExits() && TxtQuotationNo.ValidControl(ActiveControl))
            {
                TxtQuotationNo.WarningMessage("SALES GRN NUMBER IS REQUIRED FOR CHALLAN");
            }
        }
    ...
    private void TxtOrder_KeyDown(object sender, KeyEventArgs e)
    {
        ...
        else if (e.KeyData is Keys.Enter)
        {
            ...
            {
                TxtOrder.WarningMessage("SALES GRN NUMBER IS REQUIRED FOR CHALLAN");
                return;
            }

            if (TxtQuotationNo.Enabled)
            {
                TxtQuotationNo.Focus();
            }
            else
    ...
    private void TxtOrder_Validating(object sender, CancelEventArgs e)
    {
        if (TxtOrder.IsBlankOrEmpty() && TxtOrder.Enabled && ObjGlobal.SalesOrderMandatory)
        {
            ...
            {
                TxtOrder.WarningMessage("SALES ORDER NUMBER IS REQUIRED FOR CHALLAN");
                return;
            }
        }
    }
    ...
    {
        TxtBillTermAmount.Text = TxtBillTermAmount.GetDecimalString();
        LblTotalNetAmount.Text = (LblTotalBasicAmount.GetDecimal() + TxtBillTermAmount.GetDecimal()).GetDecimalString();
        LblTotalLocalNetAmount.Text = TxtCurrencyRate.GetDecimal() > 1
            ? (LblTotalNetAmount.GetDecimal() * TxtCurrencyRate.GetDecimal()).GetDecimalString()
            : LblTotalNetAmount.Text;
        LblNumberInWords.Text = ClsMoneyConversion.MoneyConversion(LblTotalLocalNetAmount.Text);
        if (_actionTag.IsValueExits() && TxtBillTermAmount.Enabled && !TxtTermAmount.Enabled)
        {
            if (ActiveControl == DGrid)
            {
                DGrid.Focus();
                return;
            }
            if (!TxtBasicAmount.Visible)
            {
    ...
    private void BtnBillingTerm_Click(object sender, EventArgs e)
    {
        ...
        var result = new FrmTermCalculation(_dtBillTerm.RowsCount() > 0 ? "UPDATE" : "SAVE", LblTotalBasicAmount.Text, false, "SB", 0, 0, _dtBillTerm);
        result.ShowDialog();
    ...
    private void OnTxtTermAmountOnEnter(object _, EventArgs e)
    {
        ...
        var result = new FrmTermCalculation(tag, TxtBasicAmount.Text, true, "SB", _productId, serialNo, existingTerm, TxtQty.Text);
        result.ShowDialog();
    ...
    {
        if (_getKeys.KeyChar is (char)Keys.Escape)
        {
            DGrid.Focus();
            return;
        }
    ...
    #endregion --------------- SALES CHALLAN ENTRY ---------------

    // METHOD FOR THIS FORM

    #region --------------- METHOD FOR THIS FORM ---------------

    private int SaveSalesChallan()
    {
        try
        {
            if (_actionTag is "SAVE" && TxtVno.IsDuplicate("SC", _actionTag))
            {
                TxtVno.Text = TxtVno.GetCurrentVoucherNo("SC", _numberScheme);
            }
            _salesChallan.ScMaster.SC_Invoice = TxtVno.Text;
            _salesChallan.ScMaster.Invoice_Date = DateTime.Parse(MskDate.Text);
            _salesChallan.ScMaster.Invoice_Miti = MskMiti.Text;
            _salesChallan.ScMaster.Ref_Vno = TxtRefVno.Text;
            _salesChallan.ScMaster.Ref_Miti = MskRefDate.Text;
            _salesChallan.ScMaster.Ref_Date = MskRefDate.GetEnglishDate(MskRefDate.Text).GetDateTime();
            _salesChallan.ScMaster.Invoice_Time = DateTime.Now;
            _salesChallan.ScMaster.Customer_Id = _ledgerId;

            _salesChallan.ScMaster.Party_Name = _dtPartyInfo.Rows.Count > 0 ? _dtPartyInfo.Rows[0]["PartyName"].ToString() : string.Empty;
            _salesChallan.ScMaster.Vat_No = _dtPartyInfo.Rows.Count > 0 ? _dtPartyInfo.Rows[0]["VatNo"].ToString() : string.Empty;
            _salesChallan.ScMaster.Contact_Person = _dtPartyInfo.Rows.Count > 0 ? _dtPartyInfo.Rows[0]["ContactPerson"].ToString() : string.Empty;
            _salesChallan.ScMaster.Mobile_No = _dtPartyInfo.Rows.Count > 0 ? _dtPartyInfo.Rows[0]["Mob"].ToString() : string.Empty;
            _salesChallan.ScMaster.Address = _dtPartyInfo.Rows.Count > 0 ? _dtPartyInfo.Rows[0]["Address"].ToString() : string.Empty;
            _salesChallan.ScMaster.ChqNo = _dtPartyInfo.Rows.Count > 0 ? _dtPartyInfo.Rows[0]["ChequeNo"].ToString() : string.Empty;
            _salesChallan.ScMaster.ChqDate = _dtPartyInfo.Rows.Count > 0 && _dtPartyInfo.Rows[0]["ChequeNo"].IsValueExits() ? _dtPartyInfo.Rows[0]["ChequeDate"].GetDateTime() : DateTime.Now;

            _salesChallan.ScMaster.Invoice_Type = CmbInvoiceType.SelectedValue.ToString();
            _salesChallan.ScMaster.Invoice_Mode = "Challan";
            _salesChallan.ScMaster.Payment_Mode = CmbPaymentMode.SelectedValue.ToString();
            _salesChallan.ScMaster.DueDays = TxtDueDays.Text.GetInt();
            _salesChallan.ScMaster.DueDate = MskDueDays.MaskCompleted ? MskDueDays.GetEnglishDate(MskDueDays.Text).GetDateTime() : DateTime.Now;
            _salesChallan.ScMaster.Agent_Id = _agentId;
            _salesChallan.ScMaster.Subledger_Id = _subLedgerId;

            _salesChallan.ScMaster.SO_Invoice = TxtOrder.Text;
            _salesChallan.ScMaster.SO_Date = _mskOrderDate.GetDateTime();

            _salesChallan.ScMaster.QOT_Invoice = TxtQuotationNo.Text;
            _salesChallan.ScMaster.QOT_Date = MskQuotationDate.GetDateTime();

            _salesChallan.ScMaster.Cls1 = _departmentId;
            _salesChallan.ScMaster.Cls2 = 0;
            _salesChallan.ScMaster.Cls3 = 0;
            _salesChallan.ScMaster.Cls4 = 0;
            _salesChallan.ScMaster.Cur_Id = _currencyId > 0 ? _currencyId : ObjGlobal.SysCurrencyId;
            _salesChallan.ScMaster.Cur_Rate = TxtCurrencyRate.Text.GetDecimal() > 0 ? TxtCurrencyRate.Text.GetDecimal() : 1;
            _salesChallan.ScMaster.CounterId = 0;
            _salesChallan.ScMaster.B_Amount = LblTotalBasicAmount.Text.GetDecimal();
            _salesChallan.ScMaster.T_Amount = TxtBillTermAmount.Text.GetDecimal();
            _salesChallan.ScMaster.N_Amount = LblTotalNetAmount.Text.GetDecimal();
            _salesChallan.ScMaster.LN_Amount = LblTotalLocalNetAmount.Text.GetDecimal();
            _salesChallan.ScMaster.Tender_Amount = 0;
            _salesChallan.ScMaster.Return_Amount = 0;
            _salesChallan.ScMaster.V_Amount = 0;
            _salesChallan.ScMaster.Tbl_Amount = 0;
            _salesChallan.ScMaster.Action_Type = _actionTag;
            _salesChallan.ScMaster.R_Invoice = false;
            _salesChallan.ScMaster.No_Print = 0;
            _salesChallan.ScMaster.In_Words = LblNumberInWords.Text;
            _salesChallan.ScMaster.Remarks = TxtRemarks.Text;
            _salesChallan.ScMaster.Audit_Lock = false;
            _salesChallan.ScMaster.GetView = DGrid;
            _salesChallan.ScMaster.dtBTerm = _dtBillTerm;
            _salesChallan.ScMaster.dtPTerm = _dtProductTerm;

            _salesChallan.ScMaster.PAttachment1 = PAttachment1.Image.ConvertImage();
            _salesChallan.ScMaster.PAttachment2 = PAttachment2.Image.ConvertImage();
            _salesChallan.ScMaster.PAttachment3 = PAttachment3.Image.ConvertImage();
            _salesChallan.ScMaster.PAttachment4 = PAttachment4.Image.ConvertImage();
            _salesChallan.ScMaster.PAttachment5 = PAttachment5.Image.ConvertImage();

            _salesChallan.SbOther.SB_Invoice = TxtVno.Text;
            _salesChallan.SbOther.Transport = TxtTransport.Text;
            _salesChallan.SbOther.VechileNo = TxtVechileNo.Text;
            _salesChallan.SbOther.BiltyNo = TxtBiltyNo.Text;
            _salesChallan.SbOther.Package = TxtPackage.Text;
            _salesChallan.SbOther.BiltyDate = MskBiltyDate.GetDateTime();
            _salesChallan.SbOther.BiltyType = CmbBiltyType.Text;
            _salesChallan.SbOther.Driver = TxtDriver.Text;
            _salesChallan.SbOther.PhoneNo = TxtPhoneNo.Text;
            _salesChallan.SbOther.LicenseNo = TxtLicenseNo.Text;
            _salesChallan.SbOther.MailingAddress = TxtMailingAddress.Text;
            _salesChallan.SbOther.MCity = TxtMCity.Text;
            _salesChallan.SbOther.MState = TxtMState.Text;
            _salesChallan.SbOther.MCountry = TxtMCountry.Text;
            _salesChallan.SbOther.MEmail = TxtMEmail.Text;
            _salesChallan.SbOther.ShippingAddress = TxtShippingAddress.Text;
            _salesChallan.SbOther.SCity = TxtSCity.Text;
            _salesChallan.SbOther.SState = TxtSState.Text;
            _salesChallan.SbOther.SCountry = TxtSCountry.Text;
            _salesChallan.SbOther.SEmail = TxtSEmail.Text;
            _salesChallan.SbOther.ContractNo = TxtContractNo.Text;
            _salesChallan.SbOther.ContractDate = MskContractNoDate.GetDateTime();
            _salesChallan.SbOther.ExportInvoice = TxtExportInvoiceNo.Text;
            _salesChallan.SbOther.ExportInvoiceDate = MskExportInvoiceDate.GetDateTime();
            _salesChallan.SbOther.ExportInvoice = TxtVendorOrderNo.Text;
            _salesChallan.SbOther.BankDetails = TxtBankDetails.Text;
            _salesChallan.SbOther.LcNumber = TxtLCNumber.Text;
            _salesChallan.SbOther.CustomDetails = TxtCustomName.Text;

            _salesChallan.ScMaster.SyncBaseId = ObjGlobal.IsOnlineSync ? Guid.NewGuid() : Guid.Empty;
            _salesChallan.ScMaster.SyncGlobalId = ObjGlobal.IsOnlineSync ? Guid.NewGuid() : Guid.Empty;
            if (ObjGlobal.LocalOriginId != null)
            {
                _salesChallan.ScMaster.SyncOriginId = ObjGlobal.IsOnlineSync && ObjGlobal.LocalOriginId.IsValueExits()
                    ? ObjGlobal.LocalOriginId.Value
                    : Guid.Empty;
            }

            _salesChallan.ScMaster.SyncCreatedOn = DateTime.Now;
            _salesChallan.ScMaster.SyncLastPatchedOn = DateTime.Now;

            var sync = _salesChallan.ReturnSyncRowVersionVoucher("SC", TxtVno.Text);
            _salesChallan.ScMaster.SyncRowVersion = sync;

            // SALES INVOICE DETAILS
            _salesChallan.DetailsList.Clear();

            if (DGrid.Rows.Count > 0)
            {
                foreach (DataGridViewRow row in DGrid.Rows)
                {
                    var list = new SC_Details();
                    if (row.Cells["GTxtProductId"].Value.GetLong() is 0)
                    {
                        continue;
                    }
                    list.SC_Invoice = TxtVno.Text;
                    ...
                    list.QOT_Invoice = row.Cells["GTxtOrderNo"].Value.GetString();
                    list.QOT_Sno = row.Cells["GTxtOrderSno"].Value.GetInt();
                    list.SO_Invoice = row.Cells["GTxtOrderNo"].Value.GetString();
                    list.SO_SNo = row.Cells["GTxtOrderSno"].Value.GetInt();
                    ...
                    _salesChallan.DetailsList.Add(list);
                }
            }

            //SALES PRODUCT TERM
            _salesChallan.Terms.Clear();
            foreach (DataRow row in _dtProductTerm.Rows)
            {
                var list = new SC_Term()
                {
                    SC_Vno = TxtVno.Text,
            ...
            return _salesChallan.SaveSalesChallan(_actionTag);
        }
        catch (Exception ex)
        {
    ...
    {
        var dt = _master.IsExitsCheckDocumentNumbering("SC");
        ...
        {
            var wnd = new FrmNumberingScheme("SC");
    ...
    private void ClearControl()
    {
        Text = !string.IsNullOrEmpty(_actionTag)
            ? $"SALES CHALLAN DETAILS [{_actionTag}]"
            : "SALES CHALLAN DETAILS";
        TxtVno.ReadOnly = !string.IsNullOrEmpty(_actionTag) && _actionTag != "SAVE";
        TxtVno.Clear();
        if (_actionTag.Equals("SAVE")) TxtVno.GetCurrentVoucherNo("SC", _numberScheme);
        if (BtnNew.Enabled)
        {
    ...
    {
        BtnNew.Enabled = BtnEdit.Enabled = !_tagStrings.Contains(_actionTag) && !isEnable;
        BtnDelete.Enabled = BtnReverse.Enabled = BtnNew.Enabled;
        ...
        TxtQuotationNo.Enabled = BtnQuotationNo.Enabled = isEnable && ObjGlobal.SalesQuotationEnable;
        MskQuotationDate.Enabled = MskQuotationMiti.Enabled = false;
        TxtOrder.Enabled = BtnOrderNo.Enabled = isEnable && ObjGlobal.SalesOrderEnable;
        MskOrderDate.Enabled = MskOrderMiti.Enabled = false;
        CmbPaymentMode.Enabled = CmbInvoiceType.Enabled = false;
        CmbPaymentMode.Visible = CmbInvoiceType.Visible = false;
    ...
    private void EnableGridControl(bool isEnable = false)
    {
        ...
        if (isEnable)
        {
            TxtTermAmount.Enabled = _isPTermExits;
            TxtTermAmount.Visible = _isPTermExits;
        }
        else
        {
            TxtTermAmount.Enabled = false;
            TxtTermAmount.Visible = false;
        }
    ...
    {
        var dtDesign = _master.GetPrintVoucherList("SC");
        if (dtDesign.Rows.Count > 0)
        {
            var frmName = dtDesign.Rows[0]["DesignerPaper_Name"].ToString();
            var frmDp = new FrmDocumentPrint(frmName.IsValueExits() ? frmName : "Crystal", "SC", TxtVno.Text, TxtVno.Text, true)
            {
                Owner = ActiveForm
            };
            frmDp.ShowDialog();
    ...
    private void AddTextToGrid(bool isUpdate)
    {
        ...
        DGrid.Rows[iRows].Cells["GTxtQuotSno"].Value = DGrid.Rows[iRows].Cells["GTxtSNo"].Value;

        DGrid.Rows[iRows].Cells["GTxtQuotNo"].Value = TxtQuotationNo.Text;
        DGrid.Rows[iRows].Cells["GTxtQuotSno"].Value = DGrid.Rows[iRows].Cells["GTxtSNo"].Value;
    ...
    private void GetSerialNo()
    {
        ...
        for (var i = 0; i < DGrid.RowCount; i++)
        {
            ...
            DGrid.Rows[i].HeaderCell.Value = TxtSno.Text;
        }
    }
    ...
    {
        _design.GetSalesEntryGridDesign(DGrid, "SC");
    ...
    private void FillChallanInvoiceData(string voucherNo)
    {
        try
        {
            var dsSales = _entry.ReturnSalesChallanDetailsInDataSet(voucherNo);
            ...
            if (dsSales.Tables.Count > 0)
            {
                foreach (DataRow dr in dtMaster.Rows)
                {
                    ...
                    TxtQuotationNo.Text = Convert.ToString(dr["SC_Invoice"].ToString());
                ...
                if (dtDetails.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dtDetails.Rows)
                    {
                        ...
                        DGrid.Rows[iRow].Cells["GTxtQuotNo"].Value = dr["SQ_Invoice"].ToString();
                        DGrid.Rows[iRow].Cells["GTxtQuotationSno"].Value = dr["SQ_SNo"].ToString();
    ...
    private void FillQuotationData(string voucherNo)
    {
        try
        {
            var dsQuotation = _entry.ReturnSalesQuotationDetailsInDataSet(voucherNo);
            if (dsQuotation.Tables.Count <= 0)
            {
                return;
            }
            var dtMaster = dsQuotation.Tables[0];
            var dtDetails = dsQuotation.Tables[1];
            var dtProductTerm = dsQuotation.Tables[2];
            var dtBillTerm = dsQuotation.Tables[3];
            var dtTransport = dsQuotation.Tables[4];
            ...
            foreach (DataRow dr in dtMaster.Rows)
            {
                ...
                {
                    TxtQuotationNo.Text = Convert.ToString(dr["SC_Invoice"].ToString());
                    _mskQuotationDate = !string.IsNullOrEmpty(TxtQuotationNo.Text)
            ...
            if (dtDetails.Rows.Count > 0)
            {
                var iRow = 0;
                DGrid.Rows.Clear();
                DGrid.Rows.Add(dtDetails.Rows.Count);
                foreach (DataRow dr in dtDetails.Rows)
                {
                    DGrid.Rows[iRow].Cells["GTxtSNo"].Value = dr["Invoice_SNo"].ToString();
                    DGrid.Rows[iRow].Cells["GTxtProductId"].Value = dr["P_Id"].ToString();
                    ...
                    if (_actionTag != "SAVE")
                    {
                        DGrid.Rows[iRow].Cells["GTxtQuotationNo"].Value = dr["SC_Invoice"].ToString();
                        DGrid.Rows[iRow].Cells["GTxtQuotationSno"].Value = dr["Invoice_SNo"].ToString();
                        DGrid.Rows[iRow].Cells["GTxtOrderNo"].Value = dr["SO_Invoice"].ToString();
                        DGrid.Rows[iRow].Cells["GTxtOrderSno"].Value = dr["SO_SNo"].ToString();
                    }
                ...
                if (dtBillTerm.Rows.Count > 0)
                {
                    var dtBTerm = dsQuotation.Tables[3];
                    foreach (DataRow ro in dtBTerm.Rows)
                    {
                        var dataRow = _dtBillTerm.NewRow();
                        dataRow["OrderNo"] = ro["OrderNo"];
                ...
                if (dtTransport.Rows.Count > 0)
                {
                    foreach (DataRow dr in dsQuotation.Tables[4].Rows)
                    {
                        TxtTransport.Text = dr["Transport"].ToString();
                        TxtVechileNo.Text = dr["VechileNo"].ToString();
                        TxtBiltyNo.Text = dr["BiltyNo"].ToString();
                    ...
                    foreach (DataRow dr in dsSales.Tables[0].Rows)
                    {
                        ...
                        if (_actionTag != "SAVE")
                        {
                            ...
                            if (!string.IsNullOrEmpty(TxtOrder.Text.Trim()))
                                _mskOrderDate = dr["SO_Date"].GetDateString();
                            TxtQuotationNo.Text = Convert.ToString(dr["SC_Invoice"].ToString());
                            if (!string.IsNullOrEmpty(TxtQuotationNo.Text.Trim()))
                                _mskQuotationDate = dr["SC_Date"].GetDateString();
                        }
                    ...
                    if (dsSales.Tables[1].Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow dr in dsSales.Tables[1].Rows)
                        {
                            ...
                            DGrid.Rows[iRows].Cells["GTxtRate"].Value =
                                Convert.ToDouble(dr["Rate"]).ToString(ObjGlobal.SysAmountFormat);
                            DGrid.Rows[iRows].Cells["GTxtAmount"].Value = Convert.ToDouble(dr["B_Amount"])
                                .ToString(ObjGlobal.SysAmountFormat);
                            DGrid.Rows[iRows].Cells["GTxtTermAmount"].Value = Convert.ToDouble(dr["T_Amount"])
                                .ToString(ObjGlobal.SysAmountFormat);
                            DGrid.Rows[iRows].Cells["GTxtNetAmount"].Value = Convert.ToDouble(dr["N_Amount"])
                                .ToString(ObjGlobal.SysAmountFormat);
                            DGrid.Rows[iRows].Cells["GTxtAltStockQty"].Value = Convert.ToDouble(dr["AltStock_Qty"])
                                .ToString(ObjGlobal.SysQtyFormat);
                            DGrid.Rows[iRows].Cells["GTxtStockQty"].Value = Convert.ToDouble(dr["Stock_Qty"])
                                .ToString(ObjGlobal.SysQtyFormat);
                            ...
                            DGrid.Rows[iRows].Cells["GTxtQuotationNo"].Value = dr["SC_Invoice"].ToString();
                            DGrid.Rows[iRows].Cells["GTxtQuotationSno"].Value = dr["SC_SNo"].ToString();
                            DGrid.Rows[iRows].Cells["GTxtConFraction"].Value = string.Empty;
    ...
    private string _mskQuotationDate = string.Empty;
