...
public partial class uInventoryReports : XtraUserControl
{
    [Obsolete]
    public uInventoryReports(string reportText, string reportDesc, bool isSummary)
    {
        InitializeComponent();
    ...
    {
        Text = ReportText;
        var result = new FrmFilter("R", ReportDesc, ReportDesc);
        result.ShowDialog();
    ...
    public void BindSelectedReportAsync(string reportDesc, string fromDate, string toDate)
    {
        ...
        if (TemplateId > 0)
        {
            var dt = _dynamicReport.ListTemplateType(ReportDesc, TemplateId,
                ReportTabControl.SelectedTabPageIndex is 0 ? "R" : "P");
            if (dt.Rows.Count > 0)
            {
    ...
    private bool GetRegisterReports()
    {
        ...
        if (File.Exists($"{ReportDesc}.xml"))
        {
            PGrid.RestoreLayoutFromXml($"{ReportDesc}.xml", OptionsLayoutBase.FullLayout);
        }
        return gridView1.RowCount > 0;
    }
    ...
    public void ExportToExcel()
    {
        ...
        else if (fileExtenstion == ".html") gridView1.ExportToHtml(fileName);
