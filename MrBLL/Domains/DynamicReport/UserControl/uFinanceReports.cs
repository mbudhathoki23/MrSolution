...
public partial class uFinanceReports : XtraUserControl
{
    [Obsolete]
    public uFinanceReports(string reportText, string reportDesc, bool isSummary)
    {
        InitializeComponent();
    ...
    {
        Text = ReportText;
        var result = new FrmFilter(ReportDesc, "R", ReportDesc);
        result.ShowDialog();
    ...
    public void BindSelectedReportAsync(string reportDesc, string fromDate, string toDate)
    {
        ...
        ReportDesc = reportDesc.GetUpper();
        ...
        {
            "JOURNAL VOUCHER" => _dynamicReport.GetCashBankVoucherDetails(),
            "CASH BANK VOUCHER" => _dynamicReport.GetCashBankVoucherDetails(),
            "POST DATE CHEQUE" => _dynamicReport.GetPostDatedCheque(reportDesc is "POST DATE CHEQUE - ALL"
                ? "ALL"
                : "DUE"),
            "POST DATE CHEQUE - ALL" => _dynamicReport.GetPostDatedCheque(reportDesc is "POST DATE CHEQUE - ALL"
                ? "ALL"
                : "DUE"),
    ...
    {
        var result = GetRegisterReports(reportDesc.GetUpper());
    }
    ...
    private bool GetRegisterReports(string reportDesc = "")
    {
    ...
    public void SaveTemplate(string action)
    {
        try
        {
            ...
            _dynamicReport.Model.Report_Name = TxtGridReportTemplete.Text;
    ...
    public void PrintPreview()
    {
        ...
        {
            if (ReportTabControl.SelectedTabPage == TabGridReport)
            {
    ...
    private void RGrid_KeyDown(object sender, KeyEventArgs e)
    {
        ...
        {
            if (ReportDesc.Equals("POST DATE CHEQUE"))
            {
                var result = _gridView.GetFocusedRowCellValue("VOUCHER NO");
                if (result.IsValueExits())
                {
                    var voucherNo = new FrmCashBankEntry(true, result.ToString(), "PDC");
                    voucherNo.ShowDialog();
                    BindSelectedReportAsync(ReportDesc, _dynamicReport.GetReports.FromAdDate, _dynamicReport.GetReports.ToAdDate);
                }
            }
        }
    }
    ...
    public void ExportToExcel()
    {
        if (gridView1.RowCount == 0)
        {
            MessageBox.Show(@$"{ReportDesc} RECORD NOT FOUND.", ObjGlobal.Caption, MessageBoxButtons.OK, MessageBoxIcon.Information);
            return;
        }
        ...
        else if (fileExtenstion == ".rtf")
        {
            gridView1.ExportToRtf(fileName);
        }
        else if (fileExtenstion == ".html")
        {
