...
public partial class FrmDynamicRegisterReports : RibbonForm
{
    private XtraUserControl _createControl;
    private uRegisterReports _uReports;
    private string _reportDesc;

    [Obsolete]
    public FrmDynamicRegisterReports()
    {
        InitializeComponent();
        _createControl = CreateUserControl("USER CONTROL : REGISTER REPORTS");
        ElementsControl.SelectedElement = uMnuSalesMaster;
        ElementsControl.SelectedElement = uMnuPurchaseMaster;
    }

    [Obsolete]
    private XtraUserControl CreateUserControl(string text)
    {
        _uReports = new uRegisterReports(text, text.GetUpper(), false)
        {
    ...
    private void AccordionControl_SelectedElementChanged(object sender, AccordionControlElementEventArgs e)
    {
        try
        {
            if (e.Element == null || e.Element.Text is @"Sales Master" or @"Purchase Master")
            {
                return;
            }
    ...
    {
        var result = new FrmFilter(_reportDesc, _uReports.ReportTabControl.TabIndex is 1 ? "R" : "P", _reportDesc);
        result.ShowDialog();
    ...
    private void AceCustomerLedger_Click(object sender, EventArgs e)
    {
    }
}