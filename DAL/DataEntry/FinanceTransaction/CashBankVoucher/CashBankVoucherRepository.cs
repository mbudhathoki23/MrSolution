...
namespace MrDAL.DataEntry.FinanceTransaction.CashBankVoucher;

public class CashBankVoucherRepository : ICashBankVoucherRepository
{
    public CashBankVoucherRepository()
    {
        CbMaster = new CB_Master();
        CbDetailsList = new List<CB_Details>();
    ...
    public int SaveCashBankVoucher(string actionTag)
    {
        ...
        if (actionTag != "DELETE" && !actionTag.Equals("POST"))
        {
            ...
            cmdTxt.Append("\n VALUES \n");
            var index = 0;
            foreach (var cbDetails in CbDetailsList)
            {
                index++;
                cmdTxt.Append($" ('{cbDetails.Voucher_No}',{cbDetails.SNo},");
                cmdTxt.Append(cbDetails.CBLedgerId > 0 ? $"{cbDetails.CBLedgerId}," : "NULL,");
                cmdTxt.Append($"{cbDetails.Ledger_Id},");
                ...
                cmdTxt.Append($"{cbDetails.Debit}, {cbDetails.Credit},");
                cmdTxt.Append($"{cbDetails.LocalDebit},{cbDetails.LocalCredit},");
                cmdTxt.Append($"{cbDetails.Tbl_Amount.GetDecimal()},{cbDetails.V_Amount.GetDecimal()},");
                ...
                cmdTxt.Append(cbDetails.SyncBaseId.IsGuidExits() ? $"'{cbDetails.SyncBaseId}'," : $"NULL,");
                cmdTxt.Append(cbDetails.SyncGlobalId.IsGuidExits() ? $"'{cbDetails.SyncGlobalId}'," : $"NULL,");
                cmdTxt.Append(cbDetails.SyncOriginId.IsGuidExits() ? $"'{cbDetails.SyncOriginId}'," : $"NULL,");
                cmdTxt.Append($"'{cbDetails.SyncCreatedOn.GetSystemDate()}','{cbDetails.SyncLastPatchedOn.GetSystemDate()}',");
                cmdTxt.Append($"{CbMaster.SyncRowVersion.GetDecimal(true)}");
                cmdTxt.Append(index == CbDetailsList.Count ? "); \n" : "),");
            }
        }
        ...
        {
            cmdTxt.Append($@"
                UPDATE AMS.CB_Master SET VoucherMode=N'POST',ReconcileBy = '{ObjGlobal.LogInUser}',ReconcileDate= GETDATE(),Audit_Lock = 1 WHERE Voucher_No='{CbMaster.Voucher_No}';");
        }
