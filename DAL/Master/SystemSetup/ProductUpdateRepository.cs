...
public class ProductUpdateRepository : IProductUpdateRepository
{
    ...
    // RETURN MASTER VALUE THROUGH MODEL

    #region --------------- INSERT, UPDATE & DELETE ASYNC ---------------
    public async Task<NonQueryResult> UpdateProductImportAsync(IList<ImportProductEModel> rows, int branchId, string username)
    {
            ...
            try
            {
                foreach (var model in rows)
                {
                    ...
                    if (product != null)
                    {
                        await conn.ExecuteAsync(@"
								UPDATE AMS.Product SET
									...
									PType = @pProductType,
									PGrpId = @pGroupId,
									PSubGrpId = @pSubGroupId,
									PBuyRate = @pBuyRate,
									PSalesRate = @pSalesRate,
									PTax = @pTaxPercent,
									PMargin1 = @pMargin1,
									TradeRate = @pTradeRate,
									PMRP = @pMrp
								WHERE PID = @pProductId ",
                            new
                            {
                                pUnitId = model.UOMId,
                                pBarCode = model.BarCode,
                                pProductType = "I",
                                pGroupId = model.GroupId,
                                pSubGroupId = model.SubGroupId,
                                pBuyRate = model.BuyRate,
                                pSalesRate = model.SalesRate,
                                pTaxPercent = model.TaxRate,
                                pMargin1 = model.Margin,
                                pTradeRate = model.TradePrice,
                                pMrp = model.Mrp,
                    ...
                    else
                    {
                        ...
                        await conn.InsertAsync(new DatabaseModule.Master.ProductSetup.Product
                        {
                            PID = newId,
                            ...
                            PSalesRate = model.SalesRate,
                            PMargin1 = model.Margin,
                            TradeRate = model.TradePrice,
                            PMRP = model.Mrp,
                            PAlias = model.Description,
                            PShortName = model.BarCode,
                            PQtyConv = 0,
                            PAltConv = 0,
                            PValTech = "FIFO",
                            PSerialno = false,
    ...
    #endregion --------------- INSERT, UPDATE & DELETE ASYNC ---------------
