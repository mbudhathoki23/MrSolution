...
public static partial class SqlMapperExtensions
{
    ...
    {
        if (ExplicitKeyProperties.TryGetValue(type.TypeHandle, out var pi)) return pi.ToList();
    ...
    {
        if (KeyProperties.TryGetValue(type.TypeHandle, out var pi)) return pi.ToList();
 ...
 name }, AssemblyBuilderAccess.Run);
        ...
        public static T GetInterfaceProxy<T>()
        {
            ...
            if (TypeCache.TryGetValue(typeOfT, out var k)) return (T)Activator.CreateInstance(k);
