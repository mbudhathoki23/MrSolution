...
public static partial class SqlMapperExtensions
{
    ...
    {
        var type = typeof(T);
        if (!GetQueries.TryGetValue(type.TypeHandle, out var sql))
        {
        ...
        if (!type.IsInterface)
            return (await connection.QueryAsync<T>(sql, dynParams, transaction, commandTimeout)).FirstOrDefault();

        var res =
            (await connection.QueryAsync<dynamic>(sql, dynParams)).FirstOrDefault() as IDictionary<string, object>;
        ...
        IDbTransaction transaction = null, int? commandTimeout = null) where T : class
    {
        ...
        if (!GetQueries.TryGetValue(cacheType.TypeHandle, out var sql))
        {
        ...
        if (!type.IsInterface) return connection.QueryAsync<T>(sql, null, transaction, commandTimeout);
    ...
    public static Task<int> InsertAsync<T>(this IDbConnection connection, T entityToInsert,
        ...
        int? commandTimeout = null, ISqlAdapter sqlAdapter = null, string TableName = null
        ) where T : class
    {
        var type = typeof(T);
        ...
        //    else
        //    {
        //        if (sbParameterList.Length > 0)
        //        {
        //            sbParameterList.Length--;
        //            sbParameterList.Length--;
        //        }
        //    }
        //}
    ...
    public static async Task<bool> UpdateAsync<T>(this IDbConnection connection, T entityToUpdate,
        IDbTransaction transaction = null, int? commandTimeout = null) where T : class
    {
        if (entityToUpdate is IProxy proxy && !proxy.IsDirty) return false;
        ...
        var updated = await connection.ExecuteAsync(sb.ToString(), entityToUpdate, commandTimeout: commandTimeout,
...
public partial interface ISqlAdapter
{
    ...
    Task<int> InsertAsync(IDbConnection connection, IDbTransaction transaction, int? commandTimeout,
...
public partial class SqlCeServerAdapter
{
        ...
        object entityToInsert)
    {
        ...
        var r = (await connection.QueryAsync<dynamic>("SELECT @@IDENTITY Id", transaction: transaction,
...
public partial class MySqlAdapter
{
        ...
        string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert)
    {
        var cmd = $"INSERT INTO {tableName} ({columnList}) VALUES ({parameterList})";
        await connection.ExecuteAsync(cmd, entityToInsert, transaction, commandTimeout);
        var r = await connection.QueryAsync<dynamic>("SELECT LAST_INSERT_ID() Id", transaction: transaction,
