...
namespace MrDAL.Domains.Shared.DataSync.Abstractions;

public abstract class DbSyncRepositoryBase(DbSyncRepoInjectData injectData)
{
    protected readonly DbSyncRepoInjectData InjectData = injectData;
    ...
    /// <param name="repoType">The sync repository type defining what type of data to be fetched</param>
    /// <returns></returns>
    protected async Task<ListResult<T>> GetUnSynchronizedDataAsync<T>()

    {
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
                result = JsonConvert.DeserializeObject<ListResult<T>>(json);
